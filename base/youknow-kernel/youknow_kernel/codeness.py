...
def program_codeness(observations: Dict[str, Any], yk: "YOUKNOW") -> None:
    ...
    TODO: BROKEN - Uses old API (yk.entities, yk.add_entity) that doesn't exist.
    Current YOUKNOW uses yk.cat.entities and yk.add().
    Use MetaInterpreter._bootstrap() in codeness_gen.py instead.
    """
    from .core import YOUKNOW

    # First, add pattern_of_isa (root)
    if "pattern_of_isa" not in yk.entities:
        yk.add_entity(PIOEntity(
            name="pattern_of_isa",
    ...
    # Add CodePattern as a category
    if "CodePattern" not in yk.entities:
        yk.add_entity(PIOEntity(
            name="CodePattern",
    ...
    # Add each pattern type
    for name, pattern in CODE_PATTERNS.items():
        if name not in yk.entities:
            entity = pattern.to_entity(is_a=["CodePattern"])
            yk.add_entity(entity)

    # Now add the observed classes as INSTANCES of patterns
    for class_name, class_info in observations["classes"].items():
        # The class IS the patterns it uses
        is_a = class_info["patterns"] if class_info["patterns"] else ["CodePattern"]

        yk.add_entity(PIOEntity(
            name=class_name,
            description=f"Class from {observations['filename']}",
            is_a=is_a,
            metadata={
                "source": observations["filename"],
                "line": class_info["line"],
                "patterns": class_info["patterns"],
            }
        ))
