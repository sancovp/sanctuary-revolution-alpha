...
class OperandExecutor:
    ...
    async def execute_plan(self, execution_plan: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        while i < len(execution_plan):
            ...
            if step.get('role') == 'condition':
            ...
            elif step.get('role') == 'for_collection':
                # Execute collection step to get the iterable
                result = await self._execute_step(step)
                results.append(result)
                var_name = step.get('for_variable', '_item')

                # Extract collection from result
                collection = []
                if result['success']:
                    data = result.get('data', {})
                    # Try to get iterable from result
                    if isinstance(data, dict) and 'result' in data:
                        r = data['result']
                        if isinstance(r, (list, tuple)):
                            collection = list(r)
                        elif isinstance(r, dict):
                            collection = list(r.items())
                ...
                self.execution_state['for_collection'] = collection
                self.execution_state['for_variable'] = var_name
                self.execution_state['for_index'] = 0

            elif step.get('loop') == 'for_body':
                # Execute for body for each item in collection
                collection = self.execution_state.get('for_collection', [])
                var_name = step.get('for_variable', '_item')
                idx = self.execution_state.get('for_index', 0)

                if idx < len(collection):
                    # Set current item in shell session vars
                    self.shell.session_vars[var_name] = collection[idx]
                    self.execution_state['step_results'][var_name] = collection[idx]

                    result = await self._execute_step(step)
                    results.append(result)

                    # After last body step, advance index and maybe loop back
                    if i == len(execution_plan) - 1 or execution_plan[i + 1].get('loop') != 'for_body':
                        self.execution_state['for_index'] = idx + 1
                        if idx + 1 < len(collection):
                            # Loop back to first for_body step
                            cond_ref = step.get('condition_ref')
                            if cond_ref is not None:
                                # Jump back to step after collection (first body step)
                                i = cond_ref  # Will be incremented at end of loop
    ...
    def _resolve_target(self, target: str) -> Optional[str]:
        """Resolve target to coordinate (handle shortcuts and combo_nodes)."""
        # Check combo_nodes first (contains all address combinations)
        if hasattr(self.shell, 'combo_nodes') and target in self.shell.combo_nodes:
            return target

        # Check nodes and numeric_nodes
        if target in self.shell.nodes:
            return target
        if hasattr(self.shell, 'numeric_nodes') and target in self.shell.numeric_nodes:
            return target

        # Try semantic address resolution
        if hasattr(self.shell, '_resolve_semantic_address'):
            resolved = self.shell._resolve_semantic_address(target)
            if resolved != target:
                if (hasattr(self.shell, 'combo_nodes') and resolved in self.shell.combo_nodes) or resolved in self.shell.nodes:
                    return resolved

        # Check shortcuts via get_shortcuts() (not session_vars)
        shortcuts = self.shell.get_shortcuts() if hasattr(self.shell, 'get_shortcuts') else {}
