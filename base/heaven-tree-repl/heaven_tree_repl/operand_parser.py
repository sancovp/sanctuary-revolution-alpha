...
class OperandParser:
    ...
    def _parse_segment(self, segment: str) -> List[Dict[str, Any]]:
        ...
        elif segment.strip().startswith('for '):
            # Parse for structure: for variable in collection x body
            parts = segment.split()

            # Find 'in' and 'x' keywords
            in_idx = None
            x_idx = None
            for i, part in enumerate(parts):
                if part == 'in' and in_idx is None:
                    in_idx = i
                elif part == 'x' and in_idx is not None:
                    x_idx = i
                    break

            if in_idx and x_idx:
                # Extract variable name, collection, and body
                var_name = parts[1]  # for VAR in ...
                collection_parts = parts[in_idx + 1:x_idx]
                collection = ' '.join(collection_parts)
                body_parts = parts[x_idx + 1:]
                body = ' '.join(body_parts)

                # Parse collection as a step (executed to get the iterable)
                coll_step = self._parse_step(collection)
                coll_step['role'] = 'for_collection'
                coll_step['for_variable'] = var_name
                steps.append(coll_step)

                # Parse body
                body_steps = self._parse_branch(body)
                for step in body_steps:
                    step['loop'] = 'for_body'
                    step['for_variable'] = var_name
                    step['condition_ref'] = 0
                    steps.append(step)
