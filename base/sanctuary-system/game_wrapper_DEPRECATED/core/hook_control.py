...
# CONNECTS_TO: /tmp/hook_config.json (read/write) — controls which hooks are enabled, read by paia_* hooks
...
class HookControl:
    ...
    def _load() -> dict[str, bool]:
        ...
        if HOOK_CONFIG.exists():
            return json.loads(HOOK_CONFIG.read_text())
        return {h: False for h in ALL_HOOKS}

    @staticmethod
    def _save(config: dict[str, bool]) -> None:
        """Save hook config."""
        HOOK_CONFIG.write_text(json.dumps(config, indent=2))
