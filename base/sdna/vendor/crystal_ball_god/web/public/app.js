const ui = {
  ...
  runStubBtn: document.querySelector("#runStubBtn"),
...
ui.runStubBtn.addEventListener("click", async () => {
  try {
    const composed = composeCoordinateFromIncluded();
    ...
    const data = await api(`/api/spaces/${encodeURIComponent(state.activeSpace)}/llm-stub`, {
      method: "POST",
      body: pretty({ coordinate, includeNodeIds, prompt }),
    });
    ui.llmOutput.textContent = pretty(data);
    logStatus("LLM stub call completed.");
  } catch (error) {
    logStatus(error.message, true);
  }
});
