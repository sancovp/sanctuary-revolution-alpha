...
class DomainOverviewPost(RenderablePiece):
    """Domain overview narrative for Discord #overview channel (pinned first post)."""
    ...
    domain_mission: str
    pain_point: str
    prerequisites: List[str]  # things they already do but still have pain
    system_name: str
    dream_achieved: str
    benefits: List[str]

    def render(self) -> str:
        prereqs = ", ".join(self.prerequisites[:-1]) + f", AND {self.prerequisites[-1]}" if len(self.prerequisites) > 1 else self.prerequisites[0]
        benefits = ", ".join(self.benefits)

        return f"""# Overview

My journey with {self.domain} started when I recognized {self.pain_herald}, but I didn't know what the journey was until {self.inciting_incident}...

{self.first_framework_discovery}

Now I'm continuing to build on that journey to {self.domain_mission} for anyone who is frustrated they're experiencing {self.pain_point} even though they already do {prereqs}...

and that's what {self.domain} does in {self.system_name} -- {self.dream_achieved} -- so you can {benefits}.

Here are some of the 'boons' I've brought back from my journeys through the Wasteland of {self.domain}"""


class JourneySummaryPost(RenderablePiece):
    """Journey/framework summary for Discord #overview channel (one per journey)."""
    ...
    @classmethod
    def from_core(cls, core: JourneyCore) -> "JourneySummaryPost":
        return cls(
            framework_name=core.the_boon.split("\n")[0] if "\n" in core.the_boon else core.the_boon[:50],
            journey_name=core.journey_name,
            obstacle=core.obstacle,
            overcome=core.overcome,
            dream_achieved=core.accomplishment
        )
...
**OBSTACLE:** {self.obstacle}
**OVERCOME:** {self.overcome}
**DREAM ACHIEVED:** {self.dream_achieved}"""
