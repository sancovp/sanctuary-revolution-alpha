...
from carton_mcp.add_concept_tool import add_concept_tool_func, add_observation, rename_concept_func, get_observation_queue_dir
...
_TOOL_THROTTLE_SECONDS = 0.5  # Throttle between tool calls to prevent CPU burn from rapid MiniMax calls

def _throttled(fn):
    """Decorator: sleep after tool execution to prevent rapid-fire CPU burn."""
    @wraps(fn)
    def wrapper(*args, **kwargs):
        result = fn(*args, **kwargs)
        time.sleep(_TOOL_THROTTLE_SECONDS)
        return result
    return wrapper
...
def create_iteration_summary(
    ...
    Args:
        ...
        skill_candidate_category: understand|single_turn, or ""
    ...
    """
    try:
        # Normalize IDs — handle both raw IDs and full concept names
        # Old format: iteration_id="UUID_C1_N", conversation_id="UUID_C1"
        # New format: iteration_id="Iteration_2026_...", conversation_id="Conversation_2026_..."
        iter_base = iteration_id.removeprefix("Iteration_") if iteration_id.startswith("Iteration_") else iteration_id
        iter_ref = iteration_id if iteration_id.startswith("Iteration_") else f"Iteration_{iteration_id}"
        conv_ref = conversation_id if conversation_id.startswith("Conversation_") else f"Conversation_{conversation_id}"
...
def create_phase_summary(
    ...
    summary: str,
    domain: str,
    iteration_summaries_included: str,
    flight_candidate_name: str,
    flight_candidate_type: str,
    flight_candidate_description: str,
    flight_candidate_steps: str,
    flight_candidate_source_phases: str,
    flight_candidate_source_iterations: str,
) -> str:
    """Create a phase summary with flight candidate harvesting.

    Phases are logical groupings of iteration summaries detected by L2.
    ALL fields are REQUIRED. Use "" for fields with no value.

    Args:
        phase_id: Unique phase ID e.g. "A4131Cf7_..._1_1" (session_convnum_phasenum)
        conversation_id: Conversation portion e.g. "A4131Cf7_..._1"
        phase_number: Phase number within conversation e.g. "1", "2"
        summary: Narrative summary of what happened in this phase (200-400 words)
        domain: Primary domain e.g. "debugging", "infrastructure", "refactoring"
        iteration_summaries_included: Comma-separated Iteration_Summary concept names in this phase
        flight_candidate_name: Descriptive name for harvested flight, or ""
        flight_candidate_type: happy_path|recovery, or ""
            happy_path = phases literally ARE the flight steps (LLM called tools in sequence, should be replayable)
            recovery = meta-capability that emerged WITHIN phases when something went wrong (NOT the phases restated)
        flight_candidate_description: What this flight does and when to use it, or ""
        flight_candidate_steps: Pipe-separated flight steps e.g. "Step 1: Do X|Step 2: Do Y|Step 3: Do Z", or ""
        flight_candidate_source_phases: Comma-separated phase IDs where this pattern was observed, or ""
        flight_candidate_source_iterations: Comma-separated iteration IDs that contributed, or ""
    """
    try:
        # Normalize IDs — handle both raw IDs and full concept names
        conv_ref = conversation_id if conversation_id.startswith("Conversation_") else f"Conversation_{conversation_id}"
        phase_base = phase_id.removeprefix("Conversation_Phase_") if phase_id.startswith("Conversation_Phase_") else phase_id

        # Parse iteration summaries
        iter_summaries = [s.strip() for s in iteration_summaries_included.split(",") if s.strip()]
        ...
        if iter_summaries:
            relationships.append({"relationship": "contains_summary", "related": iter_summaries})

        # Create the phase concept
        concept_name = f"Conversation_Phase_{phase_base}"
        raw_result = add_concept_tool_func(
            concept_name,
            summary,
            [
                {"relationship": "is_a", "related": ["Conversation_Phase"]},
                {"relationship": "part_of", "related": [conv_ref]},
                {"relationship": "instantiates", "related": ["Conversation_Phase_Template"]},
            ] + relationships,
            desc_update_mode="replace",
            hide_youknow=True,
            shared_connection=_neo4j_conn,
        ...
        # If flight candidate provided, create that too
        if flight_candidate_name.strip():
            type_map = {
                "happy_path": "Flight_Candidate_Happy_Path",
                "recovery": "Flight_Candidate_Recovery",
            }
            parent = type_map.get(flight_candidate_type.strip(), "Flight_Candidate")

            # Parse steps into numbered relationships
            steps = [s.strip() for s in flight_candidate_steps.split("|") if s.strip()]
            source_phases = [s.strip() for s in flight_candidate_source_phases.split(",") if s.strip()]
            source_iters = [s.strip() for s in flight_candidate_source_iterations.split(",") if s.strip()]

            flight_rels = [
                {"relationship": "is_a", "related": ["Flight_Candidate"]},
                {"relationship": "part_of", "related": [parent]},
                {"relationship": "instantiates", "related": ["Flight_Candidate_Template"]},
                {"relationship": "has_domain", "related": [domain]},
                {"relationship": "has_flight_type", "related": [flight_candidate_type.strip()]},
            ]
            if source_phases:
                flight_rels.append({"relationship": "observed_in_phase", "related": [f"Conversation_Phase_{p}" for p in source_phases]})
            if source_iters:
                flight_rels.append({"relationship": "surfaced_from", "related": [f"Iteration_Summary_{s}" for s in source_iters]})

            # Steps encoded in description since they're ordered text
            steps_text = "\n".join(f"Step {i+1}: {s}" for i, s in enumerate(steps)) if steps else ""
            flight_desc = f"{flight_candidate_description}\n\n{steps_text}" if steps_text else flight_candidate_description

            flight_raw = add_concept_tool_func(
                flight_candidate_name.strip(),
                flight_desc,
                flight_rels,
                desc_update_mode="append",
                hide_youknow=True,
            ...
            result += f"\n+ Flight candidate: {flight_candidate_name.strip()} ({flight_candidate_type})"
