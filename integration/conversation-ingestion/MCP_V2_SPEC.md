...
After Phase 4a (all definition pairs tagged with emergent frameworks), Phase 5 synthesizes documents:

1. `add_or_update_emergent_framework(name, strata, description)` → creates emergent record in state (for tracking)
2. `get_emergent_pairs(emergent_name)` → returns bundled pairs for LLM to read
3. `set_emergent_document(name)` → creates JSON file at known path with skeleton, returns path. If already exists, returns "already exists at {path}, edit that file"
4. LLM uses Edit tool on JSON file to fill section values
5. `preview_emergent(name, output_path)` → loads JSON, renders via metastack, writes markdown to output_path for review
6. Repeat edit/preview until document is complete
7. When all emergents have complete documents → ready for Phase 6

### Tool: add_or_update_emergent_framework(name, strata, description, ...)
Creates or updates emergent framework record in state. This is for TRACKING - the actual content lives in JSON files.

Args:
- `name`: Framework name
- `strata`: paiab | sanctum | cave
- `description`: Short definition of what this framework IS (required)
- `part_of`, `has_parts`, `related_to`: Optional relationship fields

### Tool: list_emergent_frameworks()
Returns all emergent frameworks with:
- Name and strata
- Pair count (how many pairs assigned)
- JSON file status (exists or not)
- Canonical assignment (if any)

### Tool: get_emergent_pairs(emergent_name)
Returns:
- All pairs assigned to this emergent (across all conversations)
- Formatted for LLM to read and synthesize
- Grouped by conversation

### Tool: set_emergent_document(name)
Creates the JSON file for an emergent framework.

- Computes path: `$HEAVEN_DATA_DIR/frameworks/emergent_{name}.json`
- If file already exists: returns "already exists at {path}, edit that file"
- If not exists: creates JSON with skeleton structure, returns path

Does NOT take document content as argument. You edit the JSON file directly.

### Tool: preview_emergent(name, output_path)
Renders emergent framework to markdown for review.

- Loads JSON from known path (`$HEAVEN_DATA_DIR/frameworks/emergent_{name}.json`)
- Renders via metastack to produce markdown
- Writes to `output_path`
- Returns output_path

Use this to see what the framework looks like rendered. Edit the JSON, preview again, iterate.

### Emergent Framework JSON Structure
...
**Storage location:** `$HEAVEN_DATA_DIR/frameworks/`

**Naming convention:** `emergent_{name}.json`

One flat folder. No nested strata directories.

### What a Framework IS

"WHEN A HUMAN USES AN INFORMATION PRODUCT, what happens? They have INTERNAL FLAWS that prevent them from doing it."

"we are trying to type everything that EXCLUDES the internal aspect of the journey, ie we call this the FRAMEWORK. The thing that explains to people that there is in fact a journey that if they can figure out their own internal bullshit regarding, they can accomplish."

A framework provides the EXTERNAL pattern for achieving a dream. The external obstacle is always implied: "lack of a verified system for achieving this dream." The framework IS the overcome to that obstacle.

### The Structure of Requirements

"Requirement -> obstacle (how to turn knob) -> overcome (because support)"

The framework is made up of requirements, each with its own obstacle (how to turn the knob) and overcome (the answer, backed by support/evidence).
...
- **dream**: The end state - where you end up after applying this framework
- **proof**: The author's story/evidence that this framework works (usually how they solved it themselves)
- **terms**: Definitions of words needed to understand the framework
- **requirements**: The conditions that need to be met to achieve the dream
  - **requirement**: What needs to be true/done
  - **obstacle**: How to turn the knob (the practical problem)
  - **overcome**: The answer/method
  - **support**: Evidence of functional effects on requirement metrics
- **diagrams**: Optional mermaid/ascii diagrams
- **examples**: How it looks in practice

### Critical: The Proof is the Source

"The proof/story should recognizably contain everything that ends up being defined as terms, obstacles, and overcomes, and all the supports shouldve *happened* in the story/proof."

The proof/story is the SOURCE. Everything else is extracted from it:
- The **terms** are words that came up in the story that needed definition
- The **obstacles** are problems that were actually hit in the story
- The **overcomes** are what actually worked in the story
- The **supports** are evidence FROM the story that shows the overcomes worked

The story comes first. Then you extract the structured parts from it. The structure is a formalization of what happened in the story, not something invented separately.

That's why it's verifiable - someone can read the proof and see "yes, these terms appeared, these obstacles were hit, these overcomes were applied, and here's where the support came from."

### Why Proof Makes It Prose

"Each one should have that, otherwise it's hard to understand *how it would be 'prose' at all*"

Without the proof, you just have a list of requirements with obstacles and overcomes. That's a manual, not a framework document. The proof is what ties it together into something a human would actually want to read and trust.

---

## Phase Flow Summary

Phases complete by calling the next phase's entry tool:

| Phase | Action | Tool | Completes When |
|-------|--------|------|----------------|
| 5 | Vendor emergent JSON | `set_emergent_document(name)` | JSON skeleton created |
| 6 | Edit emergent JSON | Edit tool + `preview_emergent()` | Call Phase 7 tool |
| 7 | Vendor canonical JSON | `set_canonical_document(name, emergents)` | Call Phase 8 tool |
| 8 | Publish | `prepare_for_publish()` then `publish()` | Automations fired |

---

## Phase 5: Vendor Emergent Framework

### Tool: set_emergent_document(name)
Creates the JSON file for an emergent framework at known path.

- Path: `$HEAVEN_DATA_DIR/frameworks/emergent_{name}.json`
- If exists: returns "already exists at {path}, edit that file"
- If not: creates JSON skeleton, returns path

Phase 5 is done as soon as this tool is called successfully.

---

## Phase 6: Edit Emergent Framework

Work on the emergent JSON: fill in dream, proof, terms, requirements, diagrams, examples.

### Tool: preview_emergent(name, output_path)
Renders emergent to markdown for review.

Iterate: edit JSON → preview → edit → preview until done.

Phase 6 is done when you call the Phase 7 tool (set_canonical_document).

---

## Phase 7: Vendor & Edit Canonical Framework

### Tool: set_canonical_document(name, emergents)
Creates the JSON file for a canonical framework.

- Path: `$HEAVEN_DATA_DIR/frameworks/canonical_{name}.json`
- `emergents`: list of emergent names to include
- Creates JSON with refs to emergent JSONs

### Canonical Framework JSON Structure

A canonical composes multiple emergent frameworks into a larger journey.

**Key distinction:**
- If ONE emergent → emergent data passes through (same framework, just promoted)
- If MULTIPLE emergents → needs bridging explanation
...
- **dream**: The big dream this canonical achieves
- **proof**: Author's story for the overall journey
- **bridging_explanation**: How the emergent frameworks connect (only if multiple)
- **emergent_refs**: Paths to emergent JSON files

### Tool: preview_canonical(name, output_path)
Renders canonical + all emergents to markdown for review.

Phase 7 is done when you call the Phase 8 tool (prepare_for_publish).

---

## Phase 8: Publishing

Phase 8 publishes the canonical framework to all platforms. Single n8n webhook fires once with all content.

### What Gets Published

1. **Blog** - Full canonical markdown posted to blog platform (gets URL)
2. **Discord** - Full framework posted to private member channel (with blog URL)
3. **Twitter/X** - Announcement post with link to blog
4. **LinkedIn** - Announcement post with link to blog
5. **YouTube script** - Stays local as a file for when you're ready to record

n8n posts blog first to get URL, then posts to Discord and socials with that URL embedded.

### Tool: prepare_for_publish(canonical_name)

Validates canonical JSON is complete, then vendors derivative content JSONs.

**Validation** (same pattern as Phase 6→7 gate):
- No instruction placeholders remaining
- Required fields not empty

**Vendors these files:**
- `derivative_{canonical_name}_blog.json` - Blog post content
- `derivative_{canonical_name}_discord.json` - Discord post content
- `derivative_{canonical_name}_twitter.json` - Twitter announcement copy
- `derivative_{canonical_name}_linkedin.json` - LinkedIn announcement copy
- `derivative_{canonical_name}_youtube.json` - YouTube script (stays local)

Each derivative has instruction placeholders. Edit them to prepare the copy for each platform.

### Tool: preview_derivative(canonical_name, platform, output_path)

Renders a specific derivative to markdown for review.

`platform` is one of: `blog`, `discord`, `twitter`, `linkedin`, `youtube`

### Tool: publish(canonical_name)

Validates all derivatives complete, then fires n8n webhook.

**Validation:**
- All derivative JSONs have no placeholders
...
```json
{
  "canonical_name": "...",
  "strata": "...",
  "blog_content": "full markdown",
  "discord_content": "full markdown",
  "twitter_copy": "announcement text",
  "linkedin_copy": "announcement text"
}
```

YouTube script is NOT sent to n8n - it stays local at `$HEAVEN_DATA_DIR/frameworks/derivative_{canonical_name}_youtube.json`.

**n8n Workflow:**
1. Post blog → get URL
2. Post to Discord private channel (with blog URL)
3. Post to Twitter (with blog URL)
4. Post to LinkedIn (with blog URL)

**Environment Variable:** `N8N_PUBLISH_WEBHOOK` - URL of the n8n webhook endpoint.

This is the completion point for the whole flow.
