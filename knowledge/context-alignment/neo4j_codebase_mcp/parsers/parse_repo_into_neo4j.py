...
class DirectNeo4jExtractor:
    ...
    async def analyze_repository(self, repo_url: str, temp_dir: str = None, keep_local: bool = True):
        ...
        # Set default temp_dir to repos folder with datetime suffix if keeping locally
        if temp_dir is None:
            script_dir = Path(__file__).parent
            if keep_local:
                # Use datetime suffix for persistent storage
                from datetime import datetime
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                temp_dir = str(script_dir / "repos" / f"{repo_name}_{timestamp}")
            else:
                # Use simple name for temporary storage
        ...
        try:
            ...
            if not keep_local and os.path.exists(temp_dir):
                ...
                logger.info(f"Repository kept locally at: {temp_dir}")
    
    async def analyze_local_directory(self, local_path: str, repo_name: str):
        """Analyze local directory and create nodes/relationships in Neo4j"""
        ...
        try:
            ...
            logger.error(f"Error analyzing local directory {local_path}: {e}", exc_info=True)
            raise
    
    async def _create_graph(self, repo_name: str, modules_data: List[Dict], source_url: str = None):
        """Create all nodes and relationships in Neo4j"""
        
        async with self.driver.session() as session:
            # Create Repository node with source URL
            await session.run(
                "CREATE (r:Repository {name: $repo_name, source_url: $source_url, created_at: datetime()})",
                repo_name=repo_name, source_url=source_url
            )
...
async def main():
    ...
    try:
        ...
        repo_url = "https://github.com/sancovp/brain-agent.git"
