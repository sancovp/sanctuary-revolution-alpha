#!/usr/bin/env python3
"""
DEPRECATED: Use harness/server/http_server.py instead.

This module has been superseded by http_server.py which has:
- All the same functionality
- SSE event streaming
- Hook control
- Persona control
- Self-commands (restart, compact, inject)

This file is kept for reference only. Do not use in new deployments.
...
class PAIAHandoffExecuteRequest(BaseModel):
...
class PAIAHandoffExecuteResponse(BaseModel):
...
class PAIAHandoffMessageRequest(BaseModel):
...
@app.post("/execute")
async def execute(request: PAIAHandoffExecuteRequest) -> PAIAHandoffExecuteResponse:
    ...
    try:
        ...
        return PAIAHandoffExecuteResponse(
...
@app.post("/send")
async def send_message(request: PAIAHandoffMessageRequest) -> dict:
